...
const DIST_ZIP = path.join(DIST_DIR, "synapse-plugin.zip");
const FILES_TO_COPY = [
    "manifest.json",
...
async function zipDist() {
    // 使用系统zip命令打包dist目录
    if (fssync.existsSync(DIST_ZIP)) {
        await fs.rm(DIST_ZIP, { force: true });
    }
    execSync(`cd ${DIST_DIR} && zip -r synapse-plugin.zip ./*`, { stdio: "inherit" });
}
